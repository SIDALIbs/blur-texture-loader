...
// Global state for our texture loader to prevent infinite loading stalls
static HMODULE g_hD3DX = NULL;
static D3DXCreateTextureFromFileA_t g_pD3DXLoad = NULL;
static bool g_bD3DXInitialized = false;
...
public:
    ...
    // Trapping QueryInterface so the game can't bypass our wrapper
    STDMETHOD(QueryInterface)(THIS_ REFIID riid, void** ppvObj) { 
        HRESULT hr = realDevice->QueryInterface(riid, ppvObj); 
        if (SUCCEEDED(hr) && ppvObj && *ppvObj == realDevice) {
            *ppvObj = this; // Force the game to keep using our proxy
        }
        return hr;
    }

    STDMETHOD(CreateTexture)(THIS_ UINT Width, UINT Height, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DTexture9** ppTexture, HANDLE* pSharedHandle) {
        // Init the D3DX library ONLY ONCE to stop the white screen hang
        if (!g_bD3DXInitialized) {
            g_hD3DX = LoadLibraryA("d3dx9_43.dll");
            if (!g_hD3DX) g_hD3DX = LoadLibraryA("d3dx9_42.dll");
            if (g_hD3DX) {
                g_pD3DXLoad = (D3DXCreateTextureFromFileA_t)GetProcAddress(g_hD3DX, "D3DXCreateTextureFromFileA");
            }
            g_bD3DXInitialized = true;
        }
        ...
        if (fileAttr != INVALID_FILE_ATTRIBUTES && !(fileAttr & FILE_ATTRIBUTE_DIRECTORY)) {
            if (g_pD3DXLoad && SUCCEEDED(g_pD3DXLoad(realDevice, filePath.c_str(), ppTexture))) {
                return S_OK; // Custom texture successfully injected!
            }
        }
        
        // If no custom texture found, proceed instantly
    ...
    STDMETHOD(SetRenderState)(THIS_ D3DRENDERSTATETYPE State,
